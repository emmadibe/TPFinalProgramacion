...
void eliminarComentarioDelArchivo(char nombreArchivo[], int id)
{
    ...
    if(posicion != -1){
        ...
        printf("NO EXISTSE UN COMENTARIO CON ESE IDª\n");
...
void subMenuModificaComentario(stComentario c[], int pos)
{
    ...
    do
    {
        ...
        case 0:
            break;
        default:
            printf("No existe esa opci�n. Quiere volver a intentar? Presione 1.\n");
            scanf("%d", &option2);
        }
        system("cls");
    }
...
int usuarioYaComentoLibro(int idLibro, int idUsuario, char archivoComentarios[])
{
    ...
    {
        if(idUsuario == arrayComents[i].idUsuario)  // comprueba, libro por libro, si el titulo ya est� guardado en el archivo (no diferencia entre mayusculas y minusculas
        {
